...
function parse(address) {
  ...
  if (!detail.city) {
    ...
    if (list.length > 1) {
      list.forEach(function (str,index){
        ...
        const name = nameReg.exec(str);
        if (name&&!parse.name) {
          parse.name = str;
        }else
        if((!parse.num || str )&& str.indexOf("斤") !== -1){
          if (str.indexOf("10斤") !== -1||str.indexOf("十斤") !== -1||str.indexOf("5公斤") !== -1||str.indexOf("五公斤") !== -1) {
            detail.addr = detail.addr.replace(str, '').trim();
            parse.num = 5;
          }else if (str.indexOf("20斤") !== -1||str.indexOf("二十斤") !== -1||str.indexOf("10公斤") !== -1||str.indexOf("十公斤") !== -1) {
            detail.addr = detail.addr.replace(str, '').trim();
            parse.num = 10;
          }
        }else
        if((!parse.package || str)&&(str.indexOf("顺风") !== -1||str.indexOf("顺丰") !== -1||str.indexOf("邮政") !== -1)){
          if (str.indexOf("顺风") !== -1||str.indexOf("顺丰") !== -1) {
            detail.addr = detail.addr.replace(str, '').trim();
            parse.package = "顺丰";
      ...
      if(parse.num){
        detail.addr = detail.addr.replace(parse.num, '').trim()
      }
      if(parse.package){
        detail.addr = detail.addr.replace(parse.package,'').trim()
      }
    }
  } else {
    if (detail.name) {
  ...
  if(parse.province===parse.city){
    parse.addr = detail.city+"市"+detail.area+detail.addr;
  }else{
    parse.addr = detail.province+"省"+detail.city+"市"+detail.area+detail.addr;
  }
